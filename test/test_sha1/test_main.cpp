...
static void test_hash(const TestData* const data, const size_t size,
  ...
  uint8_t actual[decept::Hash::kSHA1.outputSize];
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(
        d.out, actual, decept::Hash::kSHA1.outputSize, msg.c_str());
  }
}

static void test_Monte(
    const char seed[decept::Hash::kSHA1.outputSize + 1],
    const char (*const data)[decept::Hash::kSHA1.outputSize + 1],
    const size_t size, const char* const msgPre) {
  uint8_t s[decept::Hash::kSHA1.outputSize];
  uint8_t s0[decept::Hash::kSHA1.outputSize];
  uint8_t s1[decept::Hash::kSHA1.outputSize];
  uint8_t s2[decept::Hash::kSHA1.outputSize];

  (void)std::memcpy(s, seed, decept::Hash::kSHA1.outputSize);
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    (void)std::memcpy(s0, s, decept::Hash::kSHA1.outputSize);
    (void)std::memcpy(s1, s, decept::Hash::kSHA1.outputSize);
    (void)std::memcpy(s2, s, decept::Hash::kSHA1.outputSize);
    uint8_t* md0 = s0;
    uint8_t* md1 = s1;
    uint8_t* md2 = s2;
    ...
    for (size_t j = 0; j < 1000; ++j) {
      ...
      uint8_t output1[decept::Hash::kSHA1.outputSize];
      uint8_t input[3 * decept::Hash::kSHA1.outputSize];
      (void)std::memcpy(input, md0, decept::Hash::kSHA1.outputSize);
      (void)std::memcpy(&input[decept::Hash::kSHA1.outputSize], md1,
                        decept::Hash::kSHA1.outputSize);
      (void)std::memcpy(&input[2 * decept::Hash::kSHA1.outputSize], md2,
                        decept::Hash::kSHA1.outputSize);
    ...
    (void)std::memcpy(s, md2, decept::Hash::kSHA1.outputSize);

    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(
        data[i], md2, decept::Hash::kSHA1.outputSize, msg.c_str());
  }
}
